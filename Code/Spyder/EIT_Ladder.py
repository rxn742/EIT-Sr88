...
import numpy as np
from scipy.constants import hbar, epsilon_0, k, m_p
import matplotlib.pyplot as plt
from scipy.stats import norm
from scipy.integrate import quad
from scipy.signal import find_peaks, peak_widths, peak_prominences
from shapely.geometry import LineString
from plotter import dig, gamma_ri, gamma_ig, pp, cp, Omega_c, Omega_p, density, lwp, lwc, kp, kc, sl, beamdiv, pd, cd
...
def transit(T):
    mean = 0.75*np.sqrt(np.pi)*v_mp(T)
    ttp = mean/pd
    ttc = mean/cd
    tt = np.zeros((9,9))
    if T == 0:
...
def doppler(v, delta_p, delta_c, mp, state_index):
    """
    ...
    """
    if state_index == (1,0):
        i = np.imag(population(delta_p-kp*v, delta_c+kc*v)[state_index]*maxwell_trans(v, mp))
    else:
        i = np.real(population(delta_p-kp*v, delta_c+kc*v)[state_index]*maxwell_trans(v, mp))
    return i

def dopplerint(delta_p, delta_c, mp, state_index):
    """
    This function generates the integrand to solve when including Doppler broadening
    Parameters
    ----------
    ...
    """
    mp = mp*beamdiv
    p_avg = quad(doppler, -3*mp, 3*mp, args=(delta_p, delta_c, mp, state_index))[0]
    return p_avg
...
def tcalc(delta_c, dmin, dmax, steps):
    ...
    gauss = input("Do you want to include a velocity distribution? \nY/N \n")
    if gauss == "Y":
        musig = input("Input oven temperature (C) or most probable transverse velocity \nT=? or v=? \n")
        musig = musig.split("=")
        if musig[0] == "v":
            print("Using velocity")
            mp = float(musig[1])
        if musig[0] == "T":
            print("Using oven temperature")
            mp = np.sqrt(3/2)*v_mp(float(musig[1]))
            print(mp)
        elem = 1,0
        for i in range(0, steps+1):
            print(count)
            dlist[i] = dmin
            rho_21_imag = dopplerint(dmin, delta_c, mp, elem)
...
def reltrans_plot(delta_c, dmin=-500e6, dmax=500e6, steps=2000):
    ...
    plt.title(r"Probe transmission against probe beam detuning")
    ax.plot(dlist/1e6, tlist2, color="orange", label="$\Omega_c=$" f"{Omega_c/1e6:.1f} $MHz$"\
            "\n" "$\Omega_p=$" f"{Omega_p/1e6:.1f} $MHz$" "\n" "$\Gamma_{ri}$" f"= {gamma_ri/(1e3*2*np.pi):.2f} $KHz$" "\n" "$\Gamma_{ig}$" f"= {gamma_ig/(1e6*2*np.pi):.2f} $MHz$" "\n"\
                    f"{gamma_ig/1e6:.1f}" r" x $10^6s^{-1}$" "\n" "$\Delta_c =$" f"{delta_c/1e6:.1f} $MHz$" "\n" f"$\gamma_p$ = {lwp/1e6:.1f} $MHz$" "\n"\
                        f"$\gamma_c$ = {lwc/1e6:.1f} $MHz$")
    ax.set_xlabel(r"$\Delta_p$ / $MHz$")
