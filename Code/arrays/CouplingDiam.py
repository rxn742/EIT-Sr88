...
ax2.set_ylabel("Coupling beam diameter (mm)")
ax2.set_zlabel("Relative Probe Transmission")
plt.title("Effect of transit time on EIT spectrum")
for i in diameters:
    diams = np.empty(det.size)
    diams.fill(i/1e-3)
    trans = np.genfromtxt(path + f"cdiam_t_{i/1e-3:.1f}mm.csv", delimiter=",")
    back = np.genfromtxt(path + f"cdiam_t_back{i/1e-3:.1f}mm.csv", delimiter=",")
...
ax3.set_xlabel("Coupling beam diameter (mm)")
