...
ax2.plot(det/(1e6), np.zeros(det.size), nodb, label= f"FWHM = {nodbFWHM:.2f} MHz")
ax2.set_xlabel(r"$\Delta_p$ ($MHz$)")
ax2.set_ylabel("Velocity Distribution width ($m/s$)")
...
for i in wlist:
    ...
    try:
        ...
        coFWHM = np.inf
    co_list.append(coFWHM)
    ax2.plot(det/(1e6), dv/np.sqrt(2), co, label= f"FWHM = {coFWHM:.2f} MHz")
